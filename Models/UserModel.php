...
class UserModel extends Model
{
    ...
    public function update_user($user_id, $email, $pseudo, $password)
    {
        $password = hash('whirlpool', $password);
        $stmt = $this->db->prepare("UPDATE Users SET `pseudo` = ?, `email` = ?, `password` = ? WHERE `id` = ?");

        return ($stmt->execute([$pseudo, $email, $password, $user_id]));
    }
    ...
    public function login($login, $password)
    {
        ...
        $user = $this->get_user('pseudo', $login);
    ...
    public function register($email, $pseudo, $password)
    {
        ...
        $stmt = $this->db->prepare("INSERT INTO `Users`
                  (`id`, `pseudo`, `email`, `password`, `profil_pic`, `active_key`)
				  VALUES (NULL, ?, ?, ?, ?, ?)");
        $this->send_confirmation_mail($email, $pseudo, $active_key);
        return ($stmt->execute([$pseudo, $email, $hash_pass, $profile_pic, $active_key]));
    }

    public function send_confirmation_mail($email, $pseudo, $key)
    {
        $to = $email;
        ...
        http://localhost/index.php/activation?log=' . urlencode($pseudo) . '&key=' . urlencode($key) . '
        ---------------
    ...
    public function user_exists($pseudo, $email = null)
    {
        if ($email != null) {
    ...
    public function change_password($email)
    {
        if ($user = $this->get_user('email', $email)) {
            ...
            $message = "Your password has been successfully updated.\nPlease log on Camagru with your new password.\nNew password: " . $new_password . " ";
    ...
    {
        $user = $this->get_user('pseudo', $pseudo);
        return (!empty($user));
    }
